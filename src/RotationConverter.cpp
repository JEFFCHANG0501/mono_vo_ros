...
void calib::euler(const cv::Mat& src, cv::Mat& dst, int argType)
{
  if((src.rows == 3) && (src.cols == 3))
  {
    ...
    {
      roll = -atan2(src.at<double>(1,0), src.at<double>(1,1));
      pitch = -CALIB_PI_2;
      yaw = 0.;
    }
    ...
    {
      roll = atan2(-src.at<double>(1,2), src.at<double>(2,2));
      pitch = asin(src.at<double>(0,2));
      yaw = atan2(-src.at<double>(0,1), src.at<double>(0,0));
    }
    ...
    dst.at<double>(0,0) = roll;
    dst.at<double>(1,0) = pitch;
    dst.at<double>(2,0) = yaw;
  }
